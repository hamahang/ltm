<script>
    ...
    $('.file_input_management_logo').fileinput({
        browseLabel: 'Browse',
    ...
    $(document).off('click', '#btn_save_form_management_template').on('click', '#btn_save_form_management_template', function ()
    {
        var formData = new FormData($("#management_template")[0]);
        console.log(formData);
        //formData.append('file_input_management_logo',  dataa.AdvertisingId);
        ...
        $.ajax({
            type: 'post',
            url: '{{ route('ltm.backend.templates.save_management_template')}}',
            dataType: 'json',
            data: formData,
            cache:false,
            async:false,
    ...
    function showLogoManager(result)
    {
        $('#show_area').html(result.logo_manager.view.medium) ;
    }
